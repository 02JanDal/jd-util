...
QJsonDocument Json::ensureDocument(const QByteArray &data)
{
	if (isBinaryJson(data)) {
		...
		if (doc.isNull()) {
			throw JsonException(QStringLiteral("Invalid JSON (binary JSON detected)"));
		}
		return doc;
	} else {
		...
		if (error.error != QJsonParseError::NoError) {
			throw JsonException(QStringLiteral("Error parsing JSON: %1").arg(error.errorString()));
		}
		return doc;
	}
}
...
{
	if (!doc.isObject()) {
		throw JsonException(what + QStringLiteral(" is not an object"));
	}
	return doc.object();
}
...
{
	if (!doc.isArray()) {
		throw JsonException(what + QStringLiteral(" is not an array"));
	}
	return doc.array();
}
...
template <>
QJsonValue Json::toJson<QDate>(const QDate &date)
{
...
{
	switch (variant.type()) {
	case QVariant::Url: return toJson(variant.toUrl());
	case QVariant::ByteArray: return toJson(variant.toByteArray());
	case QVariant::DateTime: return toJson(variant.toDateTime());
	case QVariant::Date: return toJson(variant.toDate());
	case QVariant::Uuid: return toJson(variant.toUuid());
	default: return QJsonValue::fromVariant(variant);
	}
}
...
template<> QByteArray ensureIsType<QByteArray>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (string != QString::fromLatin1(string.toLatin1())) {
		throw JsonException(what + QStringLiteral(" is not encodable as Latin1"));
	}
...
{
	if (!value.isString()) {
		throw JsonException(what + QStringLiteral(" is not a string"));
	}
...
{
	if (!value.isBool()) {
		throw JsonException(what + QStringLiteral(" is not a bool"));
	}
	return value.toBool();
}
...
{
	if (!value.isDouble()) {
		throw JsonException(what + QStringLiteral(" is not a double"));
	}
...
template<> int ensureIsType<int>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (fmod(doubl, 1) != 0) {
		throw JsonException(what + QStringLiteral(" is not an integer"));
	}
	return int(doubl);
}
...
template<> QDateTime ensureIsType<QDateTime>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (!datetime.isValid()) {
		throw JsonException(what + QStringLiteral(" is not a ISO formatted date/time value"));
	}
	return datetime;
}
...
template <> QDate ensureIsType<QDate>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (!date.isValid()) {
		throw JsonException(what + QStringLiteral(" is not a ISO formatted date value"));
	}
	return date;
}
...
template <> QTime ensureIsType<QTime>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (!time.isValid()) {
		throw JsonException(what + QStringLiteral(" is not a ISO formatted time value"));
	}
	return time;
}
...
template<> QUrl ensureIsType<QUrl>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (!url.isValid()) {
		throw JsonException(what + QStringLiteral(" is not a correctly formatted URL"));
	}
	return url;
}
...
template<> QUuid ensureIsType<QUuid>(const QJsonValue &value, const Requirement, const QString &what)
{
	...
	if (uuid.toString() != string) { // converts back => valid
		throw JsonException(what + QStringLiteral(" is not a valid UUID"));
	}
	return uuid;
}
...
template<> QVariant ensureIsType<QVariant>(const QJsonValue &value, const Requirement, const QString &what)
{
	if (value.isNull() || value.isUndefined()) {
		throw JsonException(what + QStringLiteral(" is null or undefined"));
	}
